...
app.put('/update/:id', (req, res) =>{
    ...
    const {nome_player, classe_player, level_player, ac_player, hp_player, xp_player} = req.body
    db.query(
        `UPDATE Players SET nome_player = ?, classe_player = ?, level_player = ?, ac_player = ?, hp_player = ?, xp_player = ? WHERE id = ?`,
        [nome_player, classe_player, Number(level_player), Number(ac_player), Number(hp_player), Number(xp_player), id_player],
        function(err, results, fields){
            if(err){
